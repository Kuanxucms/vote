<?php
	...
	if($_POST['votesubject']){
		...
		$type = $_POST['votetype'];
		$ismore = $_POST['ismore'];
		$result = $db->query("insert into votename (question_name, votetype, ismore) values ('$subject', '$type', '$ismore')");
		if($result){
...
<div class="div_from_aoto" style="width: 500px; margin:30px 40px;">
	...
	<h5 id="errortext"></h5>
        ...
        <DIV class="control-group">
            <label class="laber_from">补充其他</label>
            <DIV  class="controls" ><input name="ismore" type="radio" value="0" checked />否&nbsp;&nbsp;
									<input name="ismore" type="radio" value="1" />是
			<P class=help-block></P></DIV>
        </DIV>
        <DIV class="control-group">
